...
Broadcast FM is now decoded in stereo with RDS, and the info line that shows decoded
Morse in CW shows STEREO/MONO, the station name, programme type and radio text in WBFM.
Controls follow the mode: Pitch, Zero beat and the Morse line only in CW; the Stereo switch
only in WBFM.

**Stereo** (`dsp/fmstereo.py`). The 38 kHz carrier is regenerated by squaring the pilot
phasor from a complex band-pass, which doubles frequency and phase exactly, with no
phase-locked loop (a per-sample loop is far too slow in Python at 192 kHz). Every filter
has the same length, so every path has the same delay and the carrier lines up with the
signal without bookkeeping. De-emphasis moved to after the stereo matrix: on the whole
multiplex it would have flattened the subcarrier and the RDS with it. Measured separation
through the full chain: **43.9 dB**. CPU at 768 kS/s: 8% of a core.

**The stereo detector was wrong on first try.** It tested pilot *amplitude*, and on air,
with no antenna, it called stations barely above the noise stereo in 99% of blocks. The
FM discriminator turns noise into a multiplex with plenty of energy near 19 kHz: pure
noise measured 0.087 against a 0.03 threshold. That is worse than a wrong label, because
decoding stereo from a noise "pilot" demodulates noise from 23-53 kHz into the audio. It
now tests the pilot's **phase coherence** -- a real pilot is a steady tone (1.000), noise
...
published syndrome table did not match on the obvious conventions; a search found exactly
one that reproduces all five values -- the standard writes them bit-reversed, with the
